...
const statusOptions = ['', 'WAIT_ASSIGN', 'CLOSED', 'CANCELED']
const emergencyOptions = ['LOW', 'NORMAL', 'HIGH', 'URGENT']
  ...
  <main class="page">
    <section class="shell">
      ...
      <el-card shadow="never">
        <div class="filters">
          <el-select v-model="query.status" placeholder="状态" clearable @change="loadData" style="width: 140px">
            <el-option label="全部状态" value="" />
            <el-option label="待分配" value="WAIT_ASSIGN" />
            <el-option label="已关闭" value="CLOSED" />
            <el-option label="已取消" value="CANCELED" />
          </el-select>
          <el-input v-model="query.keyword" clearable placeholder="标题/地点" style="max-width: 260px" @keyup.enter="loadData" />
          ...
          <el-table-column prop="status" label="状态" width="100">
            <template #default="{ row }">
              <el-tag :type="row.status === 'CLOSED' ? 'success' : row.status === 'CANCELED' ? 'danger' : 'warning'">
                {{ row.status === 'WAIT_ASSIGN' ? '待分配' : row.status === 'CLOSED' ? '已关闭' : '已取消' }}
              </el-tag>
            </template>
          </el-table-column>
