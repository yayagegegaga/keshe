...
const statusLabels = Object.fromEntries(statuses.map(code => [code, dict(TASK_STATUS, code)]))
const typeLabels = {
  FEEDING: dict(TASK_TYPE, 'FEEDING'),
  CLEANING: dict(TASK_TYPE, 'CLEANING'),
  MEDICAL: dict(TASK_TYPE, 'MEDICAL'),
  FOLLOW_UP: dict(TASK_TYPE, 'FOLLOW_UP'),
  TEMP_RESCUE: dict(TASK_TYPE, 'TEMP_RESCUE')
}
      ...
      <el-card shadow="never">
        <el-form :inline="true">
          <el-form-item label="状态">
            <el-select v-model="query.status" clearable placeholder="全部" style="width: 160px">
              <el-option v-for="item in statuses" :key="item" :label="statusLabels[item]" :value="item" />
            </el-select>
          </el-form-item>
        ...
        <el-table v-loading="loading" :data="tasks" border>
          ...
          <el-table-column prop="taskType" label="类型" width="130">
            <template #default="{ row }">{{ typeLabels[row.taskType] || row.taskType }}</template>
          </el-table-column>
