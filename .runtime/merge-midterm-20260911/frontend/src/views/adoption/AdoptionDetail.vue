...
import { ADOPTION_STATUS, ADOPTION_OPERATION, ANIMAL_STATUS, ANIMAL_TYPE, dict } from '../../constants/dict'
        ...
        <el-card class="block" shadow="never">
          ...
          <el-table :data="detail.logs" border>
            ...
            <el-table-column prop="operationType" label="操作" min-width="130">
              <template #default="{ row }">{{ dict(ADOPTION_OPERATION, row.operationType) }}</template>
            </el-table-column>
            <el-table-column prop="oldStatus" label="原状态" min-width="130">
              <template #default="{ row }">{{ dict(ADOPTION_STATUS, row.oldStatus) }}</template>
            </el-table-column>
            <el-table-column prop="newStatus" label="新状态" min-width="130">
              <template #default="{ row }">{{ dict(ADOPTION_STATUS, row.newStatus) }}</template>
            </el-table-column>
