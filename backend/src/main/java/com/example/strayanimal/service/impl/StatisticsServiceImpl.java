...
public class StatisticsServiceImpl implements StatisticsService {
    ...
    private static final List<String> ADMIN_ROLE_CODES = List.of("ADMIN", "SUPER_ADMIN");
    ...
    public void evictOverviewCache() {
        evictOverviewCache(UserContext.currentUserId());
    }
    ...
    public void evictOverviewCache(Long userId) {
        ...
        try {
            stringRedisTemplate.delete(OVERVIEW_KEY_PREFIX + userId);
        } catch (Exception ignored) {
        }
    }

    @Override
    public void evictAdminOverviewCache() {
        try {
            List<Long> adminIds = sysRoleMapper.selectUserIdsByRoleCodes(ADMIN_ROLE_CODES);
            if (adminIds == null || adminIds.isEmpty()) {
                return;
            }
            for (Long adminId : adminIds) {
                try {
                    stringRedisTemplate.delete(OVERVIEW_KEY_PREFIX + adminId);
                } catch (Exception ignored) {
                }
            }
        } catch (Exception ignored) {
            // Redis 或数据库不可用时静默，不影响主流程
        }
    }
