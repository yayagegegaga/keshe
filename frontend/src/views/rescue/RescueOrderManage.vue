      ...
      <el-card shadow="never">
        <div class="filters">
          <el-select v-model="query.status" placeholder="状态" clearable @change="search" style="width: 140px">
            ...
            <el-option v-for="code in Object.keys(RESCUE_ORDER_STATUS)" :key="code" :label="dict(RESCUE_ORDER_STATUS, code)" :value="code" />
          </el-select>
        ...
        <el-table v-loading="loading" :data="orders" border>
          ...
          <el-table-column prop="emergencyLevel" label="紧急程度" width="100">
            <template #default="{ row }">{{ dict(EMERGENCY_LEVEL, row.emergencyLevel) }}</template>
          </el-table-column>
          <el-table-column prop="status" label="状态" width="100">
            <template #default="{ row }">
              <el-tag :type="row.status === 'CLOSED' ? 'success' : row.status === 'CANCELED' ? 'danger' : 'warning'">
                {{ dict(RESCUE_ORDER_STATUS, row.status) }}
              </el-tag>
            </template>
          </el-table-column>
        ...
        <el-form ref="formRef" :model="form" :rules="rules" label-width="96px">
          ...
          <el-form-item label="紧急程度" prop="emergencyLevel">
            <el-select v-model="form.emergencyLevel" style="width: 100%">
              <el-option v-for="item in emergencyOptions" :key="item" :label="dict(EMERGENCY_LEVEL, item)" :value="item" />
            </el-select>
          </el-form-item>
        </el-form>
        <template #footer>
