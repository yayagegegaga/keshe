    ...
    <section class="shell">
      ...
      <el-card shadow="never">
        ...
        <el-table v-loading="loading" :data="orders" border>
          ...
          <el-table-column prop="emergencyLevel" label="紧急程度" width="110"><template #default="{ row }">{{ dict(EMERGENCY_LEVEL, row.emergencyLevel) }}</template></el-table-column>
          <el-table-column prop="status" label="状态" width="140"><template #default="{ row }">{{ dict(RESCUE_ORDER_STATUS, row.status) }}</template></el-table-column>
