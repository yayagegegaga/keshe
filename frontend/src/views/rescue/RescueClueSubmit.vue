      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="96px">
        <el-form-item label="动物类型" prop="animalType">
          <el-select v-model="form.animalType">
            <el-option v-for="code in animalTypeOptions" :key="code" :label="dict(ANIMAL_TYPE, code)" :value="code" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="紧急程度" prop="emergencyLevel">
          <el-select v-model="form.emergencyLevel">
            <el-option v-for="code in emergencyOptions" :key="code" :label="dict(EMERGENCY_LEVEL, code)" :value="code" />
        </el-select>
        </el-form-item>
        <el-form-item label="现场图片">
          <div class="image-upload" @click="triggerImageSelect">
            <el-image v-if="form.imageUrl" class="image-preview" :src="form.imageUrl" fit="cover"><template #error><div class="image-placeholder"><el-icon><Plus /></el-icon><span>图片加载失败</span></div></template></el-image>
            <div v-else class="image-placeholder"><el-icon><Plus /></el-icon><span>点击选择图片</span></div>
            <div class="image-mask">{{ imageUploading ? '上传中...' : '点击更换' }}</div>
...
.image-upload { position: relative; width: 200px; height: 150px; border: 1px dashed #c0c4cc; border-radius: 8px; overflow: hidden; cursor: pointer; background: #f8fafc; }
