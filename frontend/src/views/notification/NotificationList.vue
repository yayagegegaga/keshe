...
import { NOTIFICATION_TYPE, dict } from '../../constants/dict'
      ...
      <el-card shadow="never">
        <div class="filters">
          <el-select v-model="query.type" clearable placeholder="类型" @change="loadData">
            <el-option v-for="item in types" :key="item || 'ALL'" :label="item ? dict(NOTIFICATION_TYPE, item) : '全部类型'" :value="item" />
          </el-select>
        ...
        <el-table v-loading="loading" :data="notifications" border>
          ...
          <el-table-column prop="type" label="类型" width="130"><template #default="{ row }">{{ dict(NOTIFICATION_TYPE, row.type) }}</template></el-table-column>
