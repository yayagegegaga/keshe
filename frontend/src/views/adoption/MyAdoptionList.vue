      ...
      <el-card shadow="never">
        <div class="filters">
          <el-select v-model="query.status" placeholder="状态" clearable @change="loadData">
            <el-option v-for="item in statuses" :key="item || 'ALL'" :label="item ? dict(ADOPTION_STATUS, item) : '全部状态'" :value="item" />
          </el-select>
        </div>
        <el-table v-loading="loading" :data="applications" border>
          <el-table-column prop="id" label="ID" width="70" />
          <el-table-column prop="animalName" label="动物" min-width="130" />
          <el-table-column prop="animalStatus" label="动物状态" width="120"><template #default="{ row }">{{ dict(ANIMAL_STATUS, row.animalStatus) }}</template></el-table-column>
          <el-table-column prop="status" label="申请状态" width="140"><template #default="{ row }">{{ dict(ADOPTION_STATUS, row.status) }}</template></el-table-column>
